...
checking of the tool are not listed.

Some parts of what is covered:
...
\begin{itemize}
  ...
  \item Partial functions (@head@,@fromJust@),

  \item The @risers@ example from \citep{catch},

  \item Some properties of functions, such that
        $@any p xs || any p ys@ = @any p (xs ++ ys)@$
        and the example with @filter@ and @any@ above.
\end{itemize}

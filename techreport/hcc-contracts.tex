...
that if the FOL prover says ``Yes!  This FOL term is provable'', then
the corresponding $\cal L$ program indeed satisfies the claimed contract?

To prove this claim we take a denotational approach.  We will assume a
program $P$, well-formed in a signature $\Sigma$, so that $\Sigma |-
P$. Most of what follows is an adaptation of folklore techniques to
...
contract (Figure~\ref{f:contract_spec}) is cast in terms of
\emph{operational} semantics but we re-cast it here in \emph{denotational} terms.
The two do not match perfectly but, as we dicuss in Section~\ref{sect:discussion}, 
the mis-match is unimportant.


The semantics of contracts is to a certain extaint a matter of design decisions, and previous
...
\end{array}\]}
The intersection of admissible sets is admissible so the $\Fcf$ bifunctor has a negative and positive fixpoint, and by minimal 
...
function $\ctrans{}{\Gamma}{e \in \Ct}$ which reflects the statement that the 
denotation of $e$ should belong in the denotation of $\Ct$.  We also include 
...
In terms of semantics we give an interpretation to the logical predicate $\lcf{\cdot}$ as:
...
a corresponding operational nottion of satisfcation at least for base contracts. 
...
which will both be modeled as $\bot$ in the denotational semantics that we have presented.
...
\spj{Is this section in the right place?}
...
\spj{This section may well be in the wrong place}

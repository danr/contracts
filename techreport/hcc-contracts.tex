...
given a function @f x = e@, the axiom $\forall x @.@ f(x) = t$ (where $\etrans{}{}{e} = t$) 
...
actually not true, but rather the following slightly annoying variation:
\[  \forall x @.@ x \neq bad \land x \neq \unr => f(x) = t \]
Alternatively, the axioms for the $app(\cdot,\cdot)$ combinator have to be modified
...
notice that in a call-by-name case, guards for $\unr$ and $\bad$ only need be placed in
functions that are strict in their argument, by performing pattern matching on it. 

...
\[\begin{array}{rcll}
  \dapp(d,a) & = & d_f(a)    & \text{if}\; d = \injFun{d_f} \\
             ...
             & = & \bot    & \text{otherwise} 
\end{array}\]}%
That is, application applies the summand if the function $d$ comes
...
which is entirely straightforward.

The semantics of right-hand sides $u$ is defined similarly.  The 
...
\subsection{Denotational semantics of contracts}
...
\dbrace{(x{:}\Ct_1) -> \Ct_2}_{\rho} 
 & = & \left\{ \begin{array}{rcl} d & \mid &
           \forall d_x \in \dbrace{\Ct_1}_\rho. \\
           && \quad \dapp(d,d_x) \in \dbrace{\Ct_2}_{\rho,x|->d_x}
           \end{array} \right\}
\\[1em]
...
\multicolumn{3}{l}{\text where} \\ 
   F_{\lcfZ}^{\infty} & = & \{\;d\;\mid\; d \neq \inj{bad} \text{ and } \\ 
                      &    & \quad\quad\text{ if } d = \inj{K_1^\ar\langle\oln{d}{\ar}\rangle} \text{ then } \ol{d} \in F_{\lcfZ}^{\infty} \} \\
                   & \cup & \ldots \\ 
                   & \cup & \{\;d\;\mid\;d \neq \inj{bad} \text{ and } \\ 
                   &      & \quad\quad\text{ if } d = \inj{->}(d_0) \text{ then } \\ 
                   &      & \quad\quad\text{ for all }\;d' \in F_{\lcfZ}^{\infty} \text{ it is } d_0(d') \in F_{\lcfZ}^{\infty} \}
\end{array}
$$
\caption{Denotations of contracts} \label{f:den-sem-contracts}
\end{figure}

The original specification of what it means for a function to satisfy a
...
\dv{it used to say ``diverge'' but it is not true. Ill typed ones are also there.} 
return \True\footnote{
In previous
work \cite{xc+:contracts} the base contract also 
...
However the $\CF$ contract is a little harder. Intuitively an expression is crash-free iff it cannot
cause a crash if plugged into a context which is itself crash-free. Of course this is a 
...
here we can do better because we are working with elements of $D_\infty$. Fortunately, using
...
to use minimal invariance. In addition we get the following, which will be useful later on for 
induction: 
...
\subsection{Soundness of the logic translation} 
...
be continuous or injectable into $D_\infty$). Happily this is straightforward to do:
\[\begin{array}{rcl}
   ...
   \linterp{\sel{K}{i}}(d) & = &  \roll{\bind{g}{\unroll{d}}} \\ 
     \text{where } g  & = & [\;\bot \\ 
...
If $\Sigma |- P$ then 
\[ \langle D_{\infty},{\cal I}\rangle \models \Th_\infty \land \ptrans{\Sigma}{P} \land \Th_{\lcfZ} \] 
\end{theorem} 
...
paper we have eluded a discussion about the operational semantics but we discuss this
...
hold about elements of $D_\infty$ that are not reflected in any of our axioms in $\Th_{\infty}$. 

...
\prooftree
  ...
  \end{array}
  --------------------------------------{TFVar}
  \etrans{\Sigma}{\Delta}{f[\oln{\tau}{n}]} = \formula{f_{ptr}}
  ~~~~ 
  (x{:}\tau) \in \Delta 
  --------------------------------------{TVar}
...
\subsection{Crash-freedom}\label{sect:cf}
\newcommand{\Fcf}{F_{\lcfZ}} 

Ideally we would like to define crash-freedom using the following strict bifunctor on 
admissible sets $S^{-},S^{+} \subseteq D_{\infty}$.
{\setlength{\arraycolsep}{2pt}
\[\begin{array}{rcl}
...
\begin{lemma} Assume that $\Sigma |- P$ and $\Sigma;\cdot |- e : \tau$. 
If $\langle D_\infty,{\cal I}\rangle \models \ctrans{\Sigma}{\Delta}{e \in \Ct}$ then 
$\Sigma;P |- e \in \Ct$. 
\end{lemma} 

...
For a given program $D$ and expression $e$, we will view the denotational 
semantics $\dbrace{D}^{\infty}$ and $\interp{e}{\dbrace{D}^\infty}{\cdot}$ as first-order 
...
\[\begin{array}{rcl}
   \linterp{f^{\ar}}  & = & \dlambda (d {:} \prod_{\ar}D_{\infty}) @.@ \dapp(\dbrace{D}^{\infty}(f),\oln{\pi_i(d)}{i \in 1..\ar}) \\ 
   \linterp{f_{ptr}}  & = & \dbrace{D}^{\infty}(f) \\ 
   \linterp{app}     & = & \dapp \\ 

...
Note that the laziness of our programming language helped in keeping the translation simple. Whenever we use universal quantification in the logic, we really mean any value, including $\bottom$. Very rarely do we have to add a condition that
a value is not equal to $\bottom$. Only when a function is explicitly strict in a particular argument (by doing pattern matching) do we have to do this. Also, function application in the language corresponds directly to term construction in the logic.

To deal with a (pure) strict language, we would have two choices: Either (1) we explicitly make each function strict in all its arguments, by adding extra axioms for each function definition that express what happens when you apply the function to $\bottom$, and by adding extra conditions to all the other function axioms that exclude $\bottom$; or (2) we use a monadic translation (or some variant thereof), basically turning each application site into a definedness check of the arguments. Neither method leads to a simple first-order theory that is easy to reason about by automatic provers.

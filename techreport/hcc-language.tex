...
function definitions $f_1^{\ar_1}\ldots f_n^{\ar_n}$ each associated with an {\em arity} $\alpha \geq 1$ 
...
Finally, there are some more non-essential design choices in Figure~\ref{fig:syntax} in order to 
...
well-formed in a signature $\Sigma$, so that $\Sigma |- P$. 
...
Using standard techniques from domain theory (the {\em embedding-projection} pairs methodology) we can
show the following.
...
Moreover, let a value-domain: $V_{\infty}$ be defined as:
    \[\begin{array}{ll}
             ...
             \; + \;\unitcpo_{bad} \quad
    \end{array}\]
Then the following functions also exist, each being the inverse of the 
...
cpo $\FVarCpo =>_c D_{\infty}$, we will call this $\dbrace{P}^{\infty}$ in what 
follows. 

\begin{definition}
To reduce notational overhead below, for a program with no free variables we will 
use notation $\dbrace{e}$ to mean $\interp{e}{\dbrace{P}^\infty}{\cdot}$.
\end{definition}
...
of $D_\infty$ and closed expressions, and using Pitts' minimal invariance to show that this 
...
We now switch gears and formally define the first-order language which will be 
the image of our translation of Haskell programs and the denotational semantics.
...
arity $\ar$ a set of {\em selector functions} $\sel{K}{i}$ for $i \in 1 \ldots \ar$.
Not all functions are fully applied, so to accommodate partial applications, we use 
the term $app(t,s)$. Functions that are not applied, but rather passed as first-class
will be represented as variables (or nullary functions) $f_{ptr}$. Finally we introduce
two new syntactic constructs $\unr$ and $\bad$. 
...
\multicolumn{3}{l}{\text{Terms}} \\
  s,t & ::=  & x                          & \text{Variables} \\ 
      ...
      & \mid & f_{ptr} \mid app(t,s)       & \text{Function pointers and application} \\
      & \mid & \unr \mid \bad             & \text{Unreachable, bad} \\ \\
%% \multicolumn{3}{l}{\text{Formulae}} \\ 
%%  \phi & ::=  & \lcf{t}    & \text{Cannot cause crash} \\
...
semantics $\dbrace{P}^{\infty}$ and $\dbrace{e}$ as 
$\interp{e}{\dbrace{P}^\infty}{\cdot}$ as first-order 

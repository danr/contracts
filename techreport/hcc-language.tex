...
{\em actual} source language programmers may ommit pattern matches but
...
\end{itemize}
Whe do we need the last clause? Consider the function @not@:
\begin{code}
  not :: Bool -> Bool
  not True = False
  not False = True
...
Figure~\ref{fig:contracts-minless} presents the tanslation
...
bacause it can crash if evaluated in the contex @fst (BAD, True)@.  Of course,

...
assert $min$ predicates. For instance if we are translating 
$\ctransmin{}{\Gamma}{f \notin \CF -> \CF -> \CF}$ we will get the following:
\[\begin{array}{l}
        \exists x y @.@ \ctransmin{}{\Gamma}{x \in \CF}\;\land\;\ctransmin{}{\Gamma}{y \in \CF}\;\land \\ 
          \quad min(app(app(f_{ptr},x),y))\;\land \neg\lcf{app(app(f_{ptr},x),y)}
\end{array}\] 
where $app(app(t,x),y) = \etrans{}{\Gamma}{e\;x\;y}$. The theorem prover will then attempt to find a 
model for this formula, or derive a contradiction). The $min$ predicates that are available will ``drive'' 
the application of function axioms from the assumptions allowing function applications to simplify to 
their bodies. For instance, if the original program includes a function definition $f |-> \lambda (x{:}\tau) (y{:}\tau) @.@ \True$ 
then the $min(app(f_{ptr},x),y)$ assumption will allow axiom \rulename{AxPtr} to trigger, and subsequently 
the definition of $f$ will be unfolded to eventually learn that $app(app(f_{ptr},x),y) = \True$ for which
we have that $\lcf{True}$.
...
However, in many practical examples we may have to use the truth value of a predicate to actually learn something about its
argument. For instance if the predicate $e'$ is the following predicate, 
\begin{code}
pred []     = True 
pred (y:ys) = False
\end{code}
\dv{Dan, can we make a concrete example?} 
by knowing that the value of the predicate is @False@ on some argument @x@, we can potentially learn that the
argument we are after to derive a contradiction is some cons cell @y:ys@. {\em However} we can't really learn 
this information unless we {\em also} have a @pred x@ in the $min(\cdot)$ predicate. If we do have this information, 
we can unfold the definition of @pred@ and figure out the information about @x@. Our solution to this problem to 
add a $min(t'[t/x])$ predicate that will now allow us to use information from the value of the predicate $t'[t/x]$ 
...
Can we guarantee formally in some cases the existence of a finite model of the theory $\Th_{infty}^{min}$?
...
In fact, if we have a concrete trace of failure of a contract, we can get the following:
...
In practice this means that we get a guarantee for a finite model for every failed trace that does 
not involve infinite values. We believe that this is possible to generalize to arbitrary traces 
though this might require further changes in our $min$-enabled translation.

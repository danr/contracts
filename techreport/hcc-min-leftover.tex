%% This is what used to be the min() section of the paper. Good for our own 
%% knowledge and our own remembering.
%% DV: July 2012
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In this figure, we have highlighted the parts where our theory differs compared to $\Th$ from
...
We defer a detailed discussion of the crash-freeness axiomatization until the next section.

\begin{figure}\small
\setlength{\arraycolsep}{2pt}
\[\begin{array}{c}
\ruleform{\utransmin{\Sigma}{u}{s} = \formula{\phi}} \\ \\
\begin{array}{rcl}
\utransmin{\Sigma}{e}{s}
  & = & \formula{(s = \etrans{\Sigma}{\Gamma}{e})} \\
\multicolumn{3}{l}{\utransmin{\Sigma}
...
\end{array}
\\ \\
\ruleform{\dtransmin{\Sigma}{d} = \formula{\phi}} \\ \\
\begin{array}{rcl}
  \dtrans{\Sigma}{f \;\ol{a}\;\ol{(x{:}\tau)} = u}
    & =     & \formula{\forall \ol{x} @.@ \highlight{min(f(\ol{x})} => \utransmin{\sigma}{u}{f(\ol{x})}} \\
    & \land & \formula{\forall \ol{x} @.@ \highlight{app(f_{ptr},\xs)} => f(\ol{x}){=}app(f_{ptr},\xs)} \\
\end{array}
\end{array}\]
...
%%   %% {       \setlength{\arraycolsep}{2pt} 
%%   %% \begin{array}{l}
%%   %% \utransmin{\Sigma}{\Gamma}{s \sim @case@\;e\;@of@\;\ol{K\;\ol{y}{->}e'}} = \\
%%   %% \;\;\formula{
...
to the $\utrans{}{u}{s}$ judgement, which now become $\utransmin{}{u}{s}$. Its definition
is given in Figure~\ref{fig:min-def-trans-min}. The important modification is that function definitions
and full applications of $app(\cdot,\cdot)$ on some function are unfolded only if we are interested
in the result of the function. Moreover, case expressions {\em shift} the focus of evaluation to the 
case scrutinee, hence the extra $min(t)$ predicate in the translation of @case@ expressions ($t$ is the 
FOL translation of the scrutinee $e$).
...
However, we shall later prove that, unlike $\Th$, the new theory $\ThMin$ does admit in many practical 
cases finite models along with infinite ones (such as $\langle D_{\infty},{\cal I}\rangle$), and these 
finite models are directly induced from concrete finite traces of failed programs.
...
It is useful to have a theory that admits a finite model. But what is its proving 
...
The modified contract translation is in Figure~\ref{fig:min-typing}. An important deviation compared to our 
previous contract translation in Figure~\ref{fig:contracts-minless} is the splitting of the contract 
translation to a {\em positive variant} $\ctransmin{}{\Gamma}{e \in \Ct}$ and a 
{\em negative variant} $\ctransmin{}{\Gamma}{e \notin \Ct}$. The reason for this is a subtle threading of 
$min(\cdot)$ predicates. 

Our goal still will be to find a contradiction to $\ctransmin{}{\Gamma}{e \notin \Ct}$, so we start by explaining
the negative variant. Observe that the formula $\ctransmin{}{\Gamma}{e \notin \Ct}$ 
always give rise to a conjunction of $min$ predicates. Consider this very simple example:
\begin{code}
g x  = True 
\end{code} 
If we wish to show that @g@ satisfies $\CF -> \CF$, we may assert the existence of a crash-free $x$, such that 
$app(@g@_{ptr},x)$ is not crash-free. However, recall that the axioms that we have generated for @g@ are: 
\[\begin{array}{l} 
   \forall x. min(app(@g@_{ptr},x)) => app(@g@_{ptr},x) = g(x) \\ 
   \forall x. min(g(x)) => g(x) = \True
\end{array}\] 
This means that, if we have no information that $min(app(@g@_{ptr},x))$ we are not in a position 
to assert that the result is $True$ which is crash-free! In fact for any value $y$ that is not 
in the $min$ predicate there is a countermodel, e.g. $g(y) = \bad$. To avoid this problem, the 
formula $\ctransmin{}{\Gamma}{e \notin \Ct}$ inserts enough $min$ predicates in order to ``drive'' 
the application of function axioms from the assumptions. In our example this manifests as: 
\[\begin{array}{l} 
   \ctransmin{}{}{g \notin \CF -> \CF} = \\
  \exists x @.@ \ctransmin{}{}{x \in \CF} \land min(app(@g@_{ptr},x)) \land \\ 
  \quad\;\; \neg\lcf{app(@g@_{ptr},x)}
\end{array}\] 
...
many delicate design choices, and we describe here the choices that led us to the current definitions.
Let us consider the base case for predicates. One naive approach would be to simply assign: 
\[\begin{array}{l}
...
a theorem prover will try to come up with some crash free $x$ such that $app(@id@_{ptr},x) = \unr \lor app(@id@_{ptr},x) = True$
and such that $@f@\;x$ is not crash-free. However, observe that the value $False$ will satisfy the above requirements:
We do not have available a predicate $min(app(@id@_{ptr},x))$ and hence we cannot reduce $app(@id@_{ptr},x)$ to $x$ and 
learn that $x$ can only be $\unr$ or $True$. Hence the formula has become satisfiable whereas this simple function 
really does belong in the denotational semantics of its contract, a serious weakness in our proving capacity.
...
are actually interested in the value $t$ above. If not, it is unlikely that any information about
...
\paragraph{Soundness}
Our design choices have been derived very carefully from our intuition about 
evaluation traces as well as practical experience from examples, but there is actually a large 
...
\begin{lemma}\label{lem:min-model} If $\langle M,\calI\rangle$ is a model of $\Th \land \ptrans{}{P}^{min}$ then the same model, extended with
$\calI_m(min)(d) = true$ for every $d \in M$, is a model of $\ThMin \land \ptrans{}{P}^{min}$. 
\end{lemma}
However the next theorem is the most important result about minimization. 
\begin{theorem}[Soundness of min translation]\label{thm:min-soundness} If $\ThMin \land \dtrans{}{P}^{min} |- \neg \ctransmin{}{}{e \notin \Ct}$ then 
                   $\Th \dtrans{}{P} |- \ctrans{}{}{e \in \Ct}$ and hence $\dbrace{\Ct}(\dbrace{e})$.
\end{theorem}
\begin{proof}
Pick a model $\langle M, \calI\rangle$ of $\Th \land \dtrans{}{P}$ and extend its interpretation 
so that $\calI(min)(d) = true$. By Lemma~\ref{lem:min-model} this means that the the extended model is a model of 
$\ThMin \land \ptrans{}{P}^{min}$ and hence this is also a model of $\neg \ctransmin{}{}{e \notin \Ct}$. 
By Lemma~\ref{lem:contract-min} (first case) this is also a model of $\ctrans{}{}{e \in \Ct}$.
\end{proof} 
Theorem~\ref{thm:min-soundness} shows that we may simply generate $\ThMin \dtrans{}{P}^{min} \land \neg \ctransmin{}{}{e \notin \Ct}$
and ask a SAT-solver for a model of this formula. If it is unsatisfiable, then its negation is valid and from the theorem we learn 
that $\dbrace{\Ct}(\dbrace{e})$.
...
better than the $min$-less version: For most programs in our testsuite, the theorem prover
finish on average faster when a contract is provable, and a finite model checker almost always finds a 
finite counter model in the case where there exists a counterexample in the $min$-enabled translation.
...
can be found in the Appendix). Importantly: observe that despite being stricter, the $\SDownarrow$ relation 
does {\em not} cause more @BAD@ values to be thrown (even if some subterms can potentially result to @BAD@ values).
...
\[ \langle S^{min},{\calI}^{min}\rangle \models \ThMin \land \ptrans{}{P}^{min} \]

...
--    Not Not{} -> False
    ...
    -- Otherwise, recurse
   -- And x y     -> True -- if invariant x then invariant y else False
    Or x       -> True -- if invariant x then invariant y else False
    Not x       -> invariant x
...
-- id x = x
-- invariant_is_invariant = invariant ::: Pred invariant --> Pred id
...
neg_contr_retain_invariant_and_cf
    = neg ::: CF --> CF  `AlongWith` Pred invariant -> Pred invariant
      
      :&: Pred invariant --> CF :&: Pred invariant
      `Using`
      invariant_cf
      `Using` 
  -->  neg_contr_cf
